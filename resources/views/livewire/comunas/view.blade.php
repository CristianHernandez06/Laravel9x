			...
			<div class="card">
				<div class="card-header">
					<div style="display: flex; justify-content: space-between; align-items: center;">
						...
						@endif
						<div>
							<input wire:model='keyWord' type="text" class="form-control" name="search" id="search" placeholder="Search Comunas">
						</div>
						<div class="btn btn-sm btn-info" data-bs-toggle="modal" data-bs-target="#createDataModal">
						<i class="fa fa-plus"></i>  Add Comunas
						</div>
					</div>
				</div>

				<div class="card-body">
						@include('livewire.comunas.modals')
				<div class="table-responsive">
								...
								<td width="90">
									<div class="dropdown">
										...
										<ul class="dropdown-menu">
											...
											<li><a class="dropdown-item" onclick="confirm('Confirm Delete Comuna id {{$row->id}}? \nDeleted Comunas cannot be recovered!')||event.stopImmediatePropagation()" wire:click="destroy({{$row->id}})"><i class="fa fa-trash"></i> Delete </a></li>
										</ul>
									</div>
								</td>
							</tr>
					...
					<div class="float-end">{{ $comunas->links() }}</div>
					</div>
				</div>
                <div class="row w-100">
					<div class="col-md-3">
